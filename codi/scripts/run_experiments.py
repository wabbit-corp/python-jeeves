...
if __name__ == "__main__":
    ...
    json_res = requests.request(
        "post", "http://127.0.0.1:8000/api/convert", headers=headers, data="".join(training).encode("utf-8")
    )

    json_res = json_res.json()
    json_res["platform"] = args.platforms[0]

    requests.request("post", "http://127.0.0.1:8000/api/train", headers=headers_train_val, data=json.dumps(json_res))
    ...
    for dataset in args.validation_set:
        ...
        json_res_val = requests.request(
            "post", "http://127.0.0.1:8000/api/convert", headers=headers, data="".join(validation).encode("utf-8")
        )

        json_res_val = json_res_val.json()
        json_res_val["platform"] = args.platforms[1]
        requests.request(
            "post", "http://127.0.0.1:8000/api/validate", headers=headers_train_val, data=json.dumps(json_res_val)
        )
