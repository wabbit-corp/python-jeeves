...
from codi.api.model.input.content import Code, Emoji, Link, Multimedia
...
class TestLink(TestContent):
    def setUp(
        ...
        path: str | None = None,
        function: object | None = None,
        blocks_in: str | None = None,
    ) -> None:
        super().setUp("fixture_data/messages_with_links.json", Link.retrieve)

    def test_retrieve_message_with_one_link(self) -> None:
...
class TestCode(TestContent):
    ...
    def test_retrieve_message_with_one_code_block(self) -> None:
        self.assertEqual(len(self._blocks[0]), 1)
        self.assertEqual(self._messages[0], "This is a large message with a code block.\n__CODEBLOCK__end")

    def test_retrieve_message_with_multiple_code_blocks(self) -> None:
    ...
    def test_retrieve_message_with_no_code_blocks(self) -> None:
        self.assertEqual(len(self._blocks[2]), 0)
        self.assertEqual(self._messages[2], "There are no code blocks here! `ciao")

    def test_retrieve_message_with_code_block_in_code_block(self) -> None:
...
class TestAttachment(TestContent):
    def setUp(
        ...
        path: str | None = None,
        function: object | None = None,
        blocks_in: str = "attachments",
    ) -> None:
        super().setUp("fixture_data/messages_with_attachments.json", Attachment.retrieve_attachments, blocks_in)

    def test_retrieve_message_with_one_attachment(self) -> None:
        self.assertEqual(len(self._blocks[0]), 1)

    def test_retrieve_message_with_multiple_attachments(self) -> None:
        self.assertEqual(len(self._blocks[1]), 4)

    def test_retrieve_message_with_no_attachments(self) -> None:
        self.assertEqual(len(self._blocks[2]), 0)
...
class TestEmoji(TestContent):
    ...
    def test_retrieve_message_with_one_emoji(self) -> None:
        self.assertEqual(len(self._blocks[0]), 1)
        self.assertEqual(self._messages[0], "Hi __EMOJI__")

    def test_retrieve_message_with_multiple_emojis(self) -> None:
        self.assertEqual(len(self._blocks[1]), 2)
        self.assertEqual(self._messages[1], "Hi __EMOJI__ __EMOJI__")

    def test_retrieve_message_with_no_emojis(self) -> None:
        self.assertEqual(len(self._blocks[2]), 0)
        self.assertEqual(self._messages[2], "Hi")

    def test_retrieve_message_with_back_to_back_emojis(self) -> None:
        self.assertEqual(len(self._blocks[3]), 2)
        self.assertEqual(self._messages[3], "Hi__EMOJI____EMOJI__")

    def test_retrieve_message_with_emoji_between_colons(self) -> None:
        self.assertEqual(len(self._blocks[4]), 1)
        self.assertEqual(self._messages[4], "Hi __EMOJI__")

    def test_retrieve_message_with_back_to_back_emojis_between_colons(self) -> None:
        self.assertEqual(len(self._blocks[5]), 2)
        self.assertEqual(self._messages[5], "Hi __EMOJI____EMOJI__")

    def test_retrieve_message_with_colon_before_emoji_between_colons(self) -> None:
