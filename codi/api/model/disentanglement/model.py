...
class Model:
    ...
    def __init__(self) -> None:
        ...
        self._gold_conversations: list[int] = []
        self._pred_conversations: list[int] = []
        self._unigram_probabilities: dict[str, float] | None = None
    ...
    def _model_dir(self) -> str:
        override = os.environ.get("CODI_MODEL_DIR")
        if override:
            return os.path.expanduser(override)
        return os.path.join(os.path.dirname(__file__), "../../training/tmp/models")

    def _save(self, model_type: str | None = None) -> None:
        """
        ...
        if model_type:
            save_path = os.path.join(model_dir, f"model-{model_type}.pickle")
        else:
            save_path = os.path.join(model_dir, "model.pickle")
    ...
    def _load(self, model_type: str | None = None) -> None:
        ...
        if model_type:
            path = os.path.join(self._model_dir(), f"model-{model_type}.pickle")
            print(f"loaded: model-{model_type}.pickle")
        else:
            path = os.path.join(self._model_dir(), "model.pickle")
    ...
    ###############################################################################################################
    def _in_window(self, message_1: Message, message_2: Message) -> bool:
        """
    ...
    def _get_previous_n_pairs(
        self,
        messages: list[Message],
        message_index: int,
    ) -> tuple[list[tuple[Message, Message]], list[tuple[int, int]]]:
        """
    ...
    def _get_more_pairs_if_in_window(
        self,
        messages: list[Message],
        message_index: int,
    ) -> tuple[list[tuple[Message, Message]], list[tuple[int, int]]]:
    ...
    def _get_all_pairs(self, messages: list[Message]) -> list[Relatedness]:
        """
    ...
    def _extract_all_features(
        ...
        training: bool = True,
    ) -> tuple[list[list[Feature]], list[int]]:
        """
    ...
    @staticmethod
    def _flatten_features(feature_matrix: list[list[Feature]]) -> list[list[float | int]]:
        """
        ...
        """
        flattened_matrix: list[list[float | int]] = []
    ...
    @measure_time
    def _compute_max_entropy(self, feature_matrix: list[list[Feature]], labels: list[int]) -> None:
        """
        ...
        if classifier == "RANDOM_FOREST":
            sm = SMOTE()
            features_resampled, labels_resampled = sm.fit_resample(flattened_matrix, labels_array)
            self._trained_model = RandomForestClassifier(n_estimators=500).fit(features_resampled, labels_resampled)
        elif classifier == "LOGISTIC_REGRESSION":
            self._trained_model = LogisticRegression(random_state=0, tol=1e-14, max_iter=100000, penalty="none").fit(
                flattened_matrix, labels_array
            )
    ...
    def _predict_max_entropy(
        ...
        """
        pairs_and_predictions: dict[str, dict[str, Relatedness]] = {}
    ...
    def _get_qualities_from_clusters(
        self,
        ...
        """
        qualities: list[tuple[int, float]] = [(-1, 0.0)] * len(conversations)
    ...
    def _cluster_messages(
        self,
        pairs: dict[str, dict[str, Relatedness]],
        ...
        """
        conversations: list[Conversation] = []
        best_cluster: tuple[int, float] | None = None

        for message in tqdm.tqdm(messages.values(), desc="Clustering messages"):
            p: dict[str, Relatedness] = {}
            try:
                p = pairs[message.uuid]
            except KeyError:
            ...
            if best_cluster and best_cluster[1] > 0:
                conversations[best_cluster[0]].messages.append(message)
    ...
    ####################
    @staticmethod
    def _convert_structure(conversations: list[Conversation]) -> dict[str, Conversation]:
        """
        ...
        """
        conversations_dict: dict[str, Conversation] = {}
        ...
        for i, conversation in enumerate(conversations):
            ...
            final_conversation.messages = list(messages_dict.values())
    ...
    @staticmethod
    def get_unigram_probabilities(community: Community, range_top_words: int = 50) -> dict[str, float]:
        """
    ...
    @staticmethod
    def _get_target_conversation_labels(channel: Channel) -> list[int]:
        """
    ...
    def _set_conversation_labels(self, channel: Channel, time_sorted_messages_dict: dict[str, Message]) -> None:
        """
    ...
    def train(
        ...
        for channel in training_set.channels.values():
            if len(channel.messages) > 0:
                ...
                self._unigram_probabilities = unigram_probabilities
                ...
                feature_matrix, labels = self._extract_all_features(pairs, features, unigram_probabilities)
    ...
    def predict(
        ...
        :param retrain: Whether to retrain on the latest training set before predicting
        ...
        if not validation and not retrain and self._trained_model is None:
            raise RuntimeError(
                "No trained model found. Train first or set CODI_MODEL_DIR to a directory with model.pickle."
            )

        for channel in community.channels.values():
            if len(channel.messages) > 0:
                time_sorted_messages_dict: dict[str, Message] = dict(
                    sorted(channel.messages.items(), key=lambda message: message[1].timestamp)
                )
                time_sorted_messages = sorted(channel.messages.values(), key=lambda message: message.timestamp)

                unigram_probabilities = self.get_unigram_probabilities(community)
                self._unigram_probabilities = unigram_probabilities
                pairs = self._get_all_pairs(time_sorted_messages)
                ...
                if validation:
                    ...
                    for group in groups:
                        ...
                        self._load(group_cls().__str__().lower())
                        feature_matrix, labels = self._extract_all_features(
                            pairs, group_cls.get_group_features(), unigram_probabilities
                        )

                        (predictions, pairs_and_predictions), max_entropy_time = self._predict_max_entropy(
                            feature_matrix, pairs, group=group_cls().__str__().lower()
                        )
                        ...
                        f_score(statistics, labels, predictions, group_cls().__str__(), times=times)
                        micro_averaged_f_score_labels(
                            statistics, self._gold_conversations, self._pred_conversations, group_cls().__str__()
                        )
                ...
                if (validation and len(groups_list) > 1) or not validation:
                    start = time.time()

                    if validation or retrain:
                        assert self._training_set is not None
                ...
                if validation and len(groups_list) > 1:
